...
use crate::stream::{Action, Check, LzmaOptions, Status, Stream};
...
/// An lzma encoder, or compressor.
///
/// This structure implements a `BufRead` interface and will read uncompressed
/// data from an underlying stream and emit a stream of compressed data.
pub struct LzmaEncoder<R> {
    obj: R,
    data: Stream,
}
...
impl<R: BufRead> LzmaEncoder<R> {
    ...
    pub fn new(r: R, level: u32) -> LzmaEncoder<R> {
        ...
        LzmaEncoder::new_stream(r, stream)
    }
...
impl<R: BufRead> Read for LzmaEncoder<R> {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        loop {
            let (read, consumed, eof, ret);
            {
...
#[cfg(feature = "tokio")]
impl<R: AsyncRead + BufRead> AsyncRead for LzmaEncoder<R> {}
...
#[cfg(feature = "tokio")]
impl<R: AsyncWrite> AsyncWrite for LzmaEncoder<R> {
    fn shutdown(&mut self) -> Poll<(), io::Error> {
        self.get_mut().shutdown()
    }
}

impl<R: BufRead> LzmaDecoder<R> {
    /// Creates a new decoder which will decompress data read from the given
    /// stream.
    pub fn new(r: R) -> LzmaDecoder<R> {
        let stream = Stream::new_lzma_decoder(u64::max_value()).unwrap();
        LzmaDecoder::new_stream(r, stream)
    }

    /// Creates a new decoder which will decompress data read from the given
    /// input. All the concatenated lzma streams from input will be consumed.
...
#[cfg(feature = "tokio")]
impl<R: AsyncRead + BufRead> AsyncRead for LzmaDecoder<R> {}

impl<W: Write> Write for LzmaDecoder<W> {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        self.get_mut().write(buf)
    }
...
#[cfg(feature = "tokio")]
impl<R: AsyncWrite> AsyncWrite for LzmaDecoder<R> {
    fn shutdown(&mut self) -> Poll<(), io::Error> {
        self.get_mut().shutdown()
    }
}
